...
public class LinearProbingHashST1<Key, Value> {
    ...
    public void put2(Key key, Value val) {
        ...
        int i;
        int collisionCount = 0;
        for (i = hashTextbook(key); keys[i] != null; i = (i + 168) % m) {          //use hashTextbook function to find the start point
            if (keys[i].equals(key)) {                                             //then if  occur collision do hash function((i + 168) % m)
                vals[i] = val;
                return;
            }
            collisionCount++;
    ...
    public Value get2(Key key) {
		...
		if (key == null) throw new IllegalArgumentException("argument to get() is null");
	    for (int i = hashTextbook(key); keys[i] != null; i = (i + 168) % m)                   //use hashTextbook function to find the start point
	        if (keys[i].equals(key))                                                          //then if  occur collision do hash function((i + 168) % m)
	            return vals[i];
